    ...
    <section>
        <?php
            foreach($lessonMovieRecords as $singleMovieObject){
                //securely handle text data with htmlspecialchars() to avoid XSS
                // (Cross-Site Scripting)
                $validatedTitle = htmlspecialchars($singleMovieObject->title ?? "Unkown Title");
                $validatedRelease = htmlspecialchars($singleMovieObject->release_date ?? "N/A");
                $validatedDescription = htmlspecialchars($singleMovieObject->description ?? "No Description Avaliable");
